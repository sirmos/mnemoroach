...
echo ""
echo "next steps:"
echo "1. enable vector indexes, a public preview feature that is off by default:"
echo "   ccloud cluster sql --id <cluster-id> --execute \"SET CLUSTER SETTING feature.vector_index.enabled = true;\""
echo "2. run the schema:"
echo "   ccloud cluster sql --id <cluster-id> < db/schema.sql"
echo "3. build the connection string for your .env as COCKROACH_URL."
